...
# 前置：DNS 已配置 bot.sc5.top、admin.sc5.top → 服务器公网 IP
set -euo pipefail
...
if ! command -v docker >/dev/null 2>&1; then
  ...
  echo "Docker 已安装。若首次加组，请重新 SSH 登录后再运行本脚本后半段。"
fi
...
if command -v certbot >/dev/null 2>&1 || sudo apt-get install -y certbot python3-certbot-nginx; then
  sudo certbot --nginx -d "$DOMAIN_BOT" -d "$DOMAIN_ADMIN" --non-interactive --agree-tos -m "admin@${DOMAIN_ADMIN#*.}" || true
fi

echo ""
echo "=== 部署完成 ==="
echo "bot-server:  https://${DOMAIN_BOT}/health"
echo "admin:       https://${DOMAIN_ADMIN}/agent/"
echo "总控:        https://${DOMAIN_ADMIN}/console/"
echo ""
echo "本地 Windows 修改 config/bot.yaml:"
echo "  botServer.url: https://${DOMAIN_BOT}"
echo ""
echo "验证: ADMIN_URL=https://${DOMAIN_ADMIN} pnpm e2e:cloud"
