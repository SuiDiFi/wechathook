...
| 服务 | 命令 | 健康检查 |
|------|------|----------|
| bot-server | `pnpm start:server` | http://127.0.0.1:8788/health · **云** https://bot.sc5.top/health |
| admin | `pnpm start:admin` | http://127.0.0.1:8790/health · **云** https://admin.sc5.top/health |
| relay-bridge | `pnpm start:relay` | http://127.0.0.1:8789/health |
| gateway | `pnpm start` | http://127.0.0.1:8787/health |

| UI | URL | 账号 |
|----|-----|------|
| 萌兔总代 | http://127.0.0.1:8790/agent/ · **云** https://admin.sc5.top/agent/ | `1000` / `000000` |
...
| **B-Admin** | 萌兔 UI、srcGet/srcPost、overrides、**总控/总代 UI 规范** | `apps/admin`、`packages/plugin-config`、`data/` | rabbitr、gateway 出站 |
...
| admin + bot-server 上云部署 | D-DevOps | **已完成**（2026-07-03，`bot.sc5.top` / `admin.sc5.top`） |
